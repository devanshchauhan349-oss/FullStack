...
app.get('/api/download/:id', protect, (req, res) => {
  ...
  if (resource.fileUrl) {
    ...
    if (fs.existsSync(filePath)) {
      const fileName =
        resource.title.replace(/[^a-z0-9]/gi, '_').toLowerCase() + '.pdf';
...
app.delete('/api/resources/:id', protect, (req, res) => {
  const index = resources.findIndex((r) => r.id === parseInt(req.params.id, 10));
...
app.post('/api/reviews/:resourceId', protect, (req, res) => {
  ...
  const existing = reviews.find(
    (r) => r.resourceId === resourceId && r.userId === req.user.id
  );
...
app.get('/api/reviews/:resourceId', (req, res) => {
  const resourceReviews = reviews.filter(
    (r) => r.resourceId === parseInt(req.params.resourceId, 10)
  );
